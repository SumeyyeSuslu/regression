...
def parse_zone(byte_list, num_vars):
    ...
    byte_start = byte_start + 4
    byte_end = byte_end + 4
    zone['NumUserdefinedFaceNeighbors'] = construct.Int32ul.parse(
        byte_list[byte_start:byte_end])

    if zone['NumUserdefinedFaceNeighbors'] !=0:
        byte_start = byte_start + 4
        byte_end = byte_end + 4
        zone['UserDefinedFaceNeighborMode'] = construct.Int32ul.parse(
            byte_list[byte_start:byte_end])

        if FeZone(zone['ZoneType']):
            byte_start = byte_start + 4
            byte_end = byte_end + 4
            zone['MiscFaceNeighbors'] = construct.Int32ul.parse(
                byte_list[byte_start:byte_end])
    ...
    if not FeZone(zone['ZoneType']):
        ...
        byte_start = byte_start + 4
        byte_end = byte_end + 4
        zone['Nodes'] = construct.Int32ul.parse(
            byte_list[byte_start:byte_end])

        byte_start = byte_start + 4
        byte_end = byte_end + 4
        zone['Elements'] = construct.Int32ul.parse(
            byte_list[byte_start:byte_end])

        byte_start = byte_start + 4
        byte_end = byte_end + 4
        zone['__ICellDim__'] = construct.Int32ul.parse(
            byte_list[byte_start:byte_end])
        byte_start = byte_start + 4
        byte_end = byte_end + 4
        zone['__JCellDim__'] = construct.Int32ul.parse(
            byte_list[byte_start:byte_end])
        byte_start = byte_start + 4
        byte_end = byte_end + 4
        zone['__KCellDim__'] = construct.Int32ul.parse(
            byte_list[byte_start:byte_end])
...
def read_zones(byte_list, zone_markers, header):
    ...
    for zone in zone_markers:
        ...
        npts = header['Zones'][zone_counter]['Nodes']
        nelems = header['Zones'][zone_counter]['Elements']
        
        for name in var_names:
            data = np.frombuffer(byte_list, dtype='float32',
                                      count=npts,
                                      offset=start_byte)
            start_byte = start_byte + 4 * npts
            zone_data[name] = data

        zone_data['mesh'] = np.frombuffer(byte_list, dtype='int32',
                                              count=nelems,
                                              offset=start_byte)
